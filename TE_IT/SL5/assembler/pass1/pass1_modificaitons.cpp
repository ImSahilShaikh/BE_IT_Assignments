...
int main()
{
	...
	bool lc_flag=0, ds_flag=0,end_flag=0,ltorg_flag=0; //lc_flag used to assign proper address to LC
	...
	while(file.get(c))
	{
		...
		if(c==' ' || c==',' || c=='\n' || c=='\t')
		{
			...
			else if(ltorg_flag)
			{
				...
				{
					littab[littab_i].index=i+1;
					littab[littab_i].literal=literal_pool[i];
					
					littab[littab_i].address=LC;
					//don't increment LC here, check again after not incrementing the value of LC here
					//LC++;
					littab_i++;
					literal_pool[i]="";
					litpool_i--;
